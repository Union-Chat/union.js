    ...
    try {
      ...
      switch (payload.op) {
        case 1: // Hello
          ...
          break;
        case 2: // Member Add
          const { server, member } = payload.d;

          if (this.servers.has(server)) {
            this.servers.get(server).members.set(member.id, member);
          }

          this.emit('serverMemberAdd', server, member);
          break;
        ...
        case 5: {
          const server = payload.d;
          this.servers.set(server.id, server);
          this.emit('serverJoin', server);
          break;
        }
        case 6: {
          const serverId = payload.d;
          if (this.servers.has(serverId)) {
            this.emit('serverLeave', this.servers.get(serverId));
            this.servers.delete(serverId);
          } else {
            this.emit('serverLeave', serverId);
          }
          break;
        }
      }
